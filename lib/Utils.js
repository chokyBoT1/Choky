...
const generateProfilePicture = async (buffer) => {
	...
	width = 300, height = jimp.getHeight() / (jimp.getWidth() / 300);
else if (jimp.getWidth() < jimp.getHeight())
	height = 720, width = jimp.getWidth() / (jimp.getHeight() / 700);
	const resized = jimp.resize(width, height);
	return {
		img: await resized.getBufferAsync(jimp_1.default.MIME_JPEG),
		preview: await resized.getBufferAsync(jimp_1.default.MIME_JPEG)
}
};
